...
`progress` is `undefined` until the first `onProgress` call, so fall
back to the file's own `size` for `max` — a 0-length bar reads as "not
started yet" rather than as broken. See
