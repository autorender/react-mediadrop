...
`react-mediadrop` owns all upload orchestration. A simple transport's job
is exactly one thing: send one file, once, report progress, resolve or
...
*other than* calling the shared retry engine (e.g. "add a retry loop
inside the transport" or "have the hook retry failed uploads itself"),
that's almost certainly wrong — point at the shared `withRetry` engine
instead of adding a second copy of this logic.
...
**`cancelGraceMs` (default `5000`)** is a safety net, not the normal path:
a well-behaved transport wires up `signal` and rejects promptly once
aborted, so cancel usually settles almost immediately. If a transport
doesn't, this timer force-frees the slot after the grace period
regardless.
