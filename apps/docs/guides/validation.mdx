...
preview during an active drag — see [Core concepts](/guides/core-concepts#drag-state).
...
silently falls back to accept-only evaluation. Don't rely on this for
correctness — the authoritative accept/reject decision is always the one
made at drop time.
