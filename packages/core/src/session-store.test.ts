...
test("createMemoryUploadSessionStore: set/get/remove round-trip", async () => {
	const store = createMemoryUploadSessionStore();
...
test("createMemoryUploadSessionStore: independent instances don't share state", async () => {
	const a = createMemoryUploadSessionStore();
	const b = createMemoryUploadSessionStore();
...
test("createBrowserUploadSessionStore: persists JSON-serializable values in localStorage", async () => {
	localStorage.clear();
	const store = createBrowserUploadSessionStore();
...
test("createBrowserUploadSessionStore: remove deletes the key", async () => {
	localStorage.clear();
	const store = createBrowserUploadSessionStore();
...
test("createBrowserUploadSessionStore: a custom prefix isolates keys from the default store", async () => {
	localStorage.clear();
	const storeA = createBrowserUploadSessionStore({ prefix: "app-a:" });
	const storeB = createBrowserUploadSessionStore({ prefix: "app-b:" });
...
test("createBrowserUploadSessionStore: get returns null for malformed stored JSON instead of throwing", async () => {
	localStorage.clear();
...
test("createBrowserUploadSessionStore: is SSR-safe when window/localStorage is unavailable", async () => {
...
test("createBrowserUploadSessionStore: set() does not throw when storage.setItem throws (e.g. quota exceeded)", async () => {
...
test("createBrowserUploadSessionStore: remove() does not throw when storage.removeItem throws", async () => {
	const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
...
test("createBrowserUploadSessionStore: swallows a throwing localStorage accessor instead of crashing", async () => {
